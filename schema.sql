-- ============================================================================
-- XLRESALE — garage sale discovery app
...
--   • sale_photos       → photos of a sale, stored in the sale-photos bucket
...
-- pg_cron creates and owns its own `cron` schema, so it CANNOT be relocated
-- into `extensions` — doing so errors out and takes the rest of this file with
-- it. Plain `create extension` is correct here.
...
create table if not exists public.sales (
  ...
  -- IANA zone for this sale's address. Hours are wall-clock local to the sale,
  -- so every comparison against now() has to go through this. Without it the
  -- auto-close job below reads 1pm Central as 1pm UTC and closes sales five
  -- hours early. The host form derives this from the Places result.
...
-- If `sales` already existed from an earlier run, bring it forward.
...
-- 3d-2. SALE_PHOTOS — uploaded photos, ordered. The file itself lives in the
--       `sale-photos` Storage bucket; this row holds the path and the order.
...
-- ----------------------------------------------------------------------------
-- Return type changed (added time_zone), so drop the old signature first —
-- CREATE OR REPLACE cannot change a function's OUT columns.
drop function if exists public.sales_near(double precision, double precision, double precision, date);
...
-- ----------------------------------------------------------------------------
--    `sale_date + closes_at` is a naive timestamp. Comparing it to now()
--    directly makes Postgres read it in the DATABASE timezone (UTC on
--    Supabase), which would close a 1pm Central sale at 8am Central. The
--    `at time zone` below anchors it to the sale's own zone first.
create or replace function public.close_expired_sales()
returns void language sql as $$
  update public.sales
  set status = 'closed'
  where status <> 'closed'
    and ((sale_date + closes_at) at time zone time_zone) < now();
$$;

-- run every 5 minutes (safe to re-run; unschedule first if it exists)
do $$
begin
  perform cron.unschedule('haul-auto-close');       -- old name, pre-rename
exception when others then null;
end $$;
do $$
begin
  perform cron.unschedule('xlresale-auto-close');
exception when others then null;
end $$;
select cron.schedule('xlresale-auto-close', '*/5 * * * *', $$ select public.close_expired_sales(); $$);
...
-- SALE_PHOTOS: readable when the sale is public; writable by the sale's host
drop policy if exists sale_photos_read on public.sale_photos;
create policy sale_photos_read on public.sale_photos
  for select to anon, authenticated using (
    exists (select 1 from public.sales s
            where s.id = sale_id and (s.listing_paid = true or s.host_id = auth.uid()))
  );
drop policy if exists sale_photos_write on public.sale_photos;
...
-- ----------------------------------------------------------------------------
-- 9b. STORAGE: policies for the `sale-photos` bucket.
--     Create the bucket FIRST in the dashboard (Storage → New bucket →
--     name `sale-photos`, Public bucket ON), then run this file.
--     Layout is  <host_id>/<sale_id>/<filename>  — the first path segment is
--     the owner, which is what these policies check.
-- ----------------------------------------------------------------------------
drop policy if exists sale_photos_object_read on storage.objects;
create policy sale_photos_object_read on storage.objects
  for select to anon, authenticated
  using (bucket_id = 'sale-photos');

drop policy if exists sale_photos_object_insert on storage.objects;
create policy sale_photos_object_insert on storage.objects
...
drop policy if exists sale_photos_object_update on storage.objects;
create policy sale_photos_object_update on storage.objects
...
drop policy if exists sale_photos_object_delete on storage.objects;
create policy sale_photos_object_delete on storage.objects
...
--   (host_id, title, description, address, location, sale_date, opens_at, closes_at, time_zone, status, listing_paid)
-- values
--   ('YOUR-PROFILE-UUID',
...
--    current_date, '08:00', '13:00', 'America/Los_Angeles', 'scheduled', true);
